...
namespace con4gis\ForumBundle\Models;
    ...
    class C4gForumPn{
        ...
        public function _save($update = false){
            if($this->validate()) {
                if ($update === false) {
                    ...
                    \Contao\Database::getInstance()->prepare($sSql)->execute(...[$this->getRecipientId(), $this->getSenderId(), $this->getSubject(), $this->getMessage(), $this->getStatus(), $this->getDtCreated()]);
                } else {
                    $sSql = "UPDATE " . self::$sTable . " SET recipient_id = ?, sender_id = ?, subject = ?, message = ?, status = ?, dt_created = ? WHERE id = ?;";
                    \Contao\Database::getInstance()->prepare($sSql)->execute(...[$this->getRecipientId(), $this->getSenderId(), $this->getSubject(), $this->getMessage(), $this->getStatus(), $this->getDtCreated(), $this->getId()]);
                }
            }else{
        ...
        private static function getMemberById($id){
            return \Contao\Database::getInstance()->prepare('SELECT id, firstname, lastname, email, username FROM tl_member WHERE id= ?;')->execute(...[$id])->fetchAssoc();
        }
        ...
        public static function getMemberByUsername($sUsername){
            ...
            return \Contao\Database::getInstance()->prepare('SELECT id, firstname, lastname, email, username FROM tl_member WHERE username = ? AND NOT disable = ?;')->execute(...[$sUsername, 1])->fetchAssoc();
        }
        ...
        public function delete(){
            try {
                \Contao\Database::getInstance()->prepare('DELETE FROM ' . self::$sTable . " WHERE id = ? LIMIT 1;")->execute(...[$this->getId()]);
                return true;
            } catch (\Exception $e) {
                return false;
            }
